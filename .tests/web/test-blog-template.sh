...
test_blog_submit_comment_endpoint_graceful_failure() {
  ...
  case "$CGI_BODY" in
    *'nostril_required'*|*'nostr_verifier_required'*|*'invalid_kind'*|*'invalid_signature'*|*'invalid_pubkey'*|*'missing_post_reference'*|*'not_found'*)
      ;;
    *)
...
test_blog_nostr_auth_endpoints_require_identity() {
  ...
  case "$CGI_BODY" in
    *'"success":true'*|*'invalid_pubkey'*|*'nostril_required'*|*'nostr_verifier_required'*) ;;
    *)
