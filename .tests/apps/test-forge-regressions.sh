...
scratch="$scratch_root/forge-regressions"
rm -rf "$scratch"
mkdir -p "$scratch"
...
if grep -E 'mktemp -d .*/wizardry-(native-workspace-bundle|workspace-bundle|iconset|native-iconset|ws-iconset)' "$backend" >/dev/null; then
