...
assert_contains "$ui" "data-arcana-item-install="
assert_contains "$ui" "'arcana:item': { label: 'Arcana module item'"
assert_matches "$ui" "function runArcanaItem\\(moduleName, itemName, options\\)"
assert_contains "$ui" "callBackend('run-action', ['arcana:item', module, item])"
