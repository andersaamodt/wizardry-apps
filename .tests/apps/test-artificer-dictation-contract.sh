...
assert_contains "$api" "  dictation_install_cancel)"
...
assert_contains "$style" ".modal-actions.modal-actions-compact.dictation-install-row {"
...
assert_contains "$ui_js" "(busy && !runningInstall);"
...
assert_contains "$ui_js" "function cancelDictationInstall()"
assert_contains "$ui_js" "function uninstallDictationSoftware()"
assert_contains "$ui_js" "function toggleDictationSoftware()"
assert_contains "$ui_js" "apiPost(\"dictation_install_start\", {}, { timeoutMs: 30000 })"
assert_contains "$ui_js" "apiPost(\"dictation_install_cancel\", { job_id: jobId }, { timeoutMs: 12000 })"
...
assert_contains "$ui_js" "buttonLabel = \"Cancel download\";"
assert_contains "$ui_js" "buttonLabel = \"Cancel install\";"
...
assert_contains "$ui_js_source" "(busy && !runningInstall);"
...
assert_contains "$ui_js_source" "apiPost(\"dictation_install_start\", {}, { timeoutMs: 30000 })"
