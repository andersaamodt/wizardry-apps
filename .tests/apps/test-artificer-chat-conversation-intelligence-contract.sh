...
assert_contains "$api" "recent_user_turns_for_conversation()"
assert_contains "$api" "chat_focus_anchors=\$(recent_user_turns_for_conversation \"\$conv_dir\" \"4\" | sed -n '1,8p')"
...
assert_contains "$api" "Thread focus anchors (recent user turns):"
