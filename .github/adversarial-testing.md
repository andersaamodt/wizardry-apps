...
- Git branch names can be valid refs while still unsafe in GitHub compare URLs; reject or encode URL delimiter characters before opening PR links.
...
- GUI help actions should dispatch through allowlisted internal help targets, not PATH lookup of user-provided command names.
- Menu arguments need the same CR/LF rejection in direct run paths as in terminal-launch display paths.
