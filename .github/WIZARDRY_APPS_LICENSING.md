...
- Keep the root `LICENSE` as the public-facing licensing overview: it must state the repo/built-in-app OWL path, the generated-project AGPL path, and the dual-license status of emission material.
- Keep `licenses/OWL-3.1.txt` as the standalone canonical OWL 3.1 text.
...
```text
This app was generated by App Forge.
This generated project is licensed under GNU AGPL-3.0-or-later.
Additional Wizardry branding terms apply; see WIZARDRY_ADDENDUM.md.
You may sell or host this app if you comply with those terms.
```
...
## External AGPL Runtime Dependencies
- The Governed Work Kernel is an external AGPL-3.0-or-later runtime dependency, not vendored Wizardry source.
- Install it through `spells/.arcana/governed-work-kernel/install-governed-work-kernel`.
- Keep Wizardry/Artificer integration at a process/file boundary such as CLI plus JSON/files.
- Do not import, vendor, link, or copy the upstream Rust crates into OWL-licensed Wizardry or Artificer code.
- User-facing Wizardry surfaces may call it `Governed Work Kernel`; third-party notices may identify the upstream project and license.
