...
- Never make weirdly tall text boxes; text inputs and textareas should use a normal, appropriate control height instead of oversized vertical padding with no clear purpose.
...
- Do not use a "left border only" accent as the main treatment for panels, callouts, errors, empty states, or login gates; it reads as overwrought and distracting rather than refined.
...
- If a resting mini-document must enter edit mode on a single click, make the resting text a visually plain native button or equivalent native hit target and immediately replace it with a `TextEditor`/native editor; verify the click path in the running app.
...
- GTK/Linux apps with backend-owned document/domain state should parse structured snapshots with JSON-GLib or another native/platform JSON parser, then rebuild native `GtkListBox`/detail widgets from that live model. Generated IR may define the shell and fallback examples, but it must not remain the primary document data source once a backend exists.
...
- The native reference starter must continue to demonstrate these conversion targets on both platforms: native source list/sidebar selection, document-feeling mini-doc editing, live backend snapshot hydration into native list/detail rows, native toolbar/headerbar, native forms/windows/preferences, `List`/`Table`/`NSOutlineView`/`GtkListBox` data surfaces instead of cards, native sidebar behavior, validated menu state, settings-window preferences, domain terminology, and platform controls for routine chrome.
