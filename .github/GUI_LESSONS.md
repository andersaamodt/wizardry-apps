...
- Open Forge row overflow menus must render through a fixed document-level portal above the catalog, keep the trigger inset from the list splitter with a stable hit target, disable row dragging while open, avoid wrapping nested action buttons in a `role=button` row, route pointerdown/mousedown/click from the visible trigger and menu rectangles directly to the intended action, and suppress repeated native events from one gesture; WebKit can route floated, draggable, edge-adjacent, or conflicting row-menu surfaces away from the visible control.
