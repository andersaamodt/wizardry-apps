...
- Open Forge row overflow menus must render through a fixed document-level portal above the catalog, disable row dragging while open, avoid wrapping nested action buttons in a `role=button` row, handle pointerdown as well as click, and route clicks from the visible menu rectangle during capture; WebKit can route clicks through floated, draggable, or conflicting row-menu surfaces instead of the visible action.
