...
- (NSString *)desktopBridgeBootstrapSource {
    ...
    if ([self isOpenCodeDesktopApp]) {
        [source appendString:
           ...
           @"    candidates = Array.prototype.filter.call(document.querySelectorAll('button, [role=\"button\"]'), function (node) {"
           @"      var rect;"
           @"      var style;"
           ...
           @"        return false;"
           @"      }"
           @"      if (rect.top < -4 || rect.top > 92 || rect.left > 168 || rect.right < 0) {"
           @"        return false;"
           @"      }"
           ...
           @"    });"
           @"    button = candidates.length ? candidates[0] : null;"
           @"    if (!root || !button || !button.parentElement) {"
           @"      return;"
           @"    }"
           @"    if (root.style && typeof root.style.setProperty === 'function') {"
           @"      root.style.setProperty('--dialog-left-margin', String(SAFE_LEFT) + 'px');"
           @"      root.style.setProperty('--sidebar-left-offset', String(SAFE_LEFT) + 'px');"
           @"      root.style.setProperty('--safe-area-left', String(SAFE_LEFT) + 'px');"
           @"    }"
           @"    wrapper = button.parentElement;"
           @"    wrapperRect = wrapper.getBoundingClientRect ? wrapper.getBoundingClientRect() : null;"
           @"    target = wrapper;"
           @"    parent = wrapper.parentElement;"
           @"    while (parent && parent !== document.body && parent !== document.documentElement) {"
           @"      targetRect = parent.getBoundingClientRect ? parent.getBoundingClientRect() : null;"
           @"      if (!targetRect) {"
           @"        break;"
           @"      }"
           @"      if (targetRect.top > 96 || targetRect.height > 120) {"
           @"        break;"
           @"      }"
           @"      if (wrapperRect && targetRect.width >= wrapperRect.width + 40 && targetRect.left <= wrapperRect.left + 6) {"
           @"        target = parent;"
           @"      }"
           ...
           @"    minLeft = SAFE_LEFT;"
           @"    delta = targetRect ? Math.ceil(minLeft - targetRect.left) : 0;"
           @"    if (!(delta > 0)) {"
           @"      return;"
           @"    }"
           @"    if (target === wrapper && wrapper.style) {"
           @"      if (!wrapper.dataset.opencodeDesktopBaseMarginLeft) {"
           @"        baseMargin = window.getComputedStyle ? parseFloat(window.getComputedStyle(wrapper).marginLeft || '0') : 0;"
           @"        if (!isFinite(baseMargin)) {"
           @"          baseMargin = 0;"
           @"        }"
           @"        wrapper.dataset.opencodeDesktopBaseMarginLeft = String(baseMargin);"
           @"      }"
           @"      baseMargin = parseFloat(wrapper.dataset.opencodeDesktopBaseMarginLeft || '0');"
           @"      if (!isFinite(baseMargin)) {"
           @"        baseMargin = 0;"
           @"      }"
           @"      wrapper.style.marginLeft = String(baseMargin + delta) + 'px';"
           @"      return;"
           @"    }"
           @"    if (target.style) {"
           @"      if (!target.dataset.opencodeDesktopBasePaddingLeft) {"
           @"        basePadding = window.getComputedStyle ? parseFloat(window.getComputedStyle(target).paddingLeft || '0') : 0;"
           @"        if (!isFinite(basePadding)) {"
           @"          basePadding = 0;"
           @"        }"
           @"        target.dataset.opencodeDesktopBasePaddingLeft = String(basePadding);"
           @"      }"
           @"      basePadding = parseFloat(target.dataset.opencodeDesktopBasePaddingLeft || '0');"
           @"      if (!isFinite(basePadding)) {"
           @"        basePadding = 0;"
           @"      }"
           @"      target.style.paddingLeft = String(basePadding + delta) + 'px';"
           @"    }"
           @"  }"
...
- (NSImage *)renderedStatusItemImageForRelayState:(NSString *)relayState busy:(BOOL)busy {
    ...
    if ([self isStonrApp]) {
        ...
        [bell lineToPoint:NSMakePoint(minX + side * 0.005, lipY)];
...
- (void)nativeOpenCodePreferences:(id)sender {
    ...
    NSString *preferencesPath = [[self.appPath stringByAppendingPathComponent:@"preferences.html"] stringByStandardizingPath];
    if (![[NSFileManager defaultManager] fileExistsAtPath:preferencesPath]) {
        NSLog(@"OpenCode preferences file not found at %@", preferencesPath);
        return;
    }
    NSURL *preferencesURL = [NSURL fileURLWithPath:preferencesPath];
    NSURLRequest *request = [NSURLRequest requestWithURL:preferencesURL];
    [self createAuxWindowWithConfiguration:nil
                                   request:request
                               windowTitle:@"OpenCode Preferences"
                                     width:920.0
                                    height:760.0];
}
        ...
        if ([program isEqualToString:@"__wizardry_host_restart_self"]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString *restartScript =
                    ...
                    @"printf '[%s] restart requested bundle=%s stage=%s pid=%s\\n' \"$(timestamp)\" \"$WIZARDRY_RESTART_BUNDLE\" \"$WIZARDRY_RESTART_STAGE\" \"$WIZARDRY_RESTART_PID\" >> \"$log_file\"; "
                    ...
                    @"    /bin/mv \"$backup_bundle\" \"$WIZARDRY_RESTART_BUNDLE\" || true; "
                    @"  fi; "
                    @"}; "
                    @"if [ -n \"$WIZARDRY_RESTART_STAGE\" ]; then "
                    @"  backup_bundle=\"${WIZARDRY_RESTART_BUNDLE}.backup.$$\"; "
                    @"  /bin/rm -rf \"$backup_bundle\"; "
                    @"  if [ -d \"$WIZARDRY_RESTART_BUNDLE\" ]; then "
                    @"    /bin/mv \"$WIZARDRY_RESTART_BUNDLE\" \"$backup_bundle\" || { "
                    @"      printf '[%s] restart failed moving current bundle aside\\n' \"$(timestamp)\" >> \"$log_file\"; "
                    @"      exit 1; "
                    @"    }; "
                    @"  fi; "
                    @"  if ! /bin/mv \"$WIZARDRY_RESTART_STAGE\" \"$WIZARDRY_RESTART_BUNDLE\"; then "
                    @"    printf '[%s] restart failed activating staged bundle\\n' \"$(timestamp)\" >> \"$log_file\"; "
                    @"    restore_original; "
                    @"    exit 1; "
                    @"  fi; "
                    @"fi; "
                    @"if ! /usr/bin/open -n \"$WIZARDRY_RESTART_BUNDLE\"; then "
                    @"  printf '[%s] restart failed reopening bundle\\n' \"$(timestamp)\" >> \"$log_file\"; "
                    @"  restore_original; "
                    @"  exit 1; "
                    @"fi; "
                    @"if [ -n \"$backup_bundle\" ] && [ -d \"$backup_bundle\" ]; then "
